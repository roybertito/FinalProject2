...
public:
	EnterScene() : trans(0), /*k("Arial", 120),*/ selected(Play), time(0), scale(1) {}
    
    void init(){
        cglow = Texture(loadImage(loadResource(RES_CONTINUEGLOW)));
        playgame = Texture(loadImage(loadResource(RES_PLAYGAME)));
        continuegame = Texture(loadImage(loadResource(RES_CONTINUE)));
        quitgame = Texture(loadImage(loadResource(RES_QUITGAME)));
        highscores = Texture(loadImage(loadResource(RES_HIGHSCORE)));
        titleTexture = Texture(loadImage(loadResource(RES_INFINITY)));
        bg = Texture(loadImage(loadResource(RES_BG)));
        planes = Texture(loadImage(loadResource(RES_PLANE)));
        play = Texture(loadImage(loadResource(RES_PLAYGLOW)));
        high = Texture(loadImage(loadResource(RES_HIGHGLOW)));
        quit = Texture(loadImage(loadResource(RES_QUITGLOW)));
		isContinuing = false;
    }
